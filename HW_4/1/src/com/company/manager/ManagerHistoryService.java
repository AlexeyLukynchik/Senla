...
public class ManagerHistoryService {
    ...
    public HistoryService[] sortHistoryService(RepositoryHistoryHotel hotel, Comparator comparator, Integer num) {
        ...
        HistoryService[] services = hotel.getHistoryHotels()[searchNumber(hotel, num)].getServices();
    ...
    public Integer searchNumber(RepositoryHistoryHotel hotel, Integer integer) {
        ...
        for (int i = 0; i < hotel.getHistoryHotels().length; i++) {

            if (hotel.getHistoryHotels()[i].getNumberRoom() == integer)
    ...
    public Integer returnCountHistory(RepositoryHistoryHotel hotel) {
        ...
        for (int i = 0; i < hotel.getHistoryHotels().length; i++) {
            ...
            for (int j = 0; j < hotel.getHistoryHotels()[i].getServices().length; j++)
                if (hotel.getHistoryHotels()[i].getServices()[j] != null) {
                    count++;
                }
